...
class statistichistoryController {
    async fetchStatistic(req, res, next){
        const { districtId, houseId, apartmentId, diseaseId, diseaseInfoId, callDatefrom, callDateto } = req.body;
        // console.log(req.body)
        if (districtId == '' && houseId == '' && apartmentId == '' && diseaseId == '' && diseaseInfoId == '' && callDatefrom && callDateto) {
            const fetchS = await model.callhistory.find().sort({ _id: -1});
            let data = []
            ...
            return res.json(data);
        } else if (districtId && houseId == '' && apartmentId == '' && diseaseId == '' && diseaseInfoId == '' && callDatefrom && callDateto) {
            const fetchS = await model.callhistory.find({districtId, "createDate": {"$gte": callDatefrom, "$lt": callDateto}}).sort({ _id: -1});
            let data = []
            for (let i = 0; i < fetchS.length; i++ ){
                let idDistrict = fetchS[i].districtId;
                let idHouse = fetchS[i].houseId;
            ...
            const fetchS = await model.callhistory.find({districtId, houseId, "createDate": {"$gte": callDatefrom, "$lt": callDateto}}).sort({ _id: -1});
            let data = []
            for (let i = 0; i < fetchS.length; i++ ){
                let idDistrict = fetchS[i].districtId;
                let idHouse = fetchS[i].houseId;
            ...
            const fetchS = await model.callhistory.find({districtId, houseId, apartmentId, "createDate": {"$gte": callDatefrom, "$lt": callDateto}}).sort({ _id: -1});
            let data = []
            for (let i = 0; i < fetchS.length; i++ ){
                let idDistrict = fetchS[i].districtId;
                let idHouse = fetchS[i].houseId;
            ...
            const fetchS = await model.callhistory.find({districtId, houseId, apartmentId, diseaseId, "createDate": {"$gte": callDatefrom, "$lt": callDateto}}).sort({ _id: -1});
            let data = []
            for (let i = 0; i < fetchS.length; i++ ){
                let idDistrict = fetchS[i].districtId;
                let idHouse = fetchS[i].houseId;
            ...
            const fetchS = await model.callhistory.find({districtId, houseId, apartmentId, diseaseId, diseaseInfoId, "createDate": {"$gte": callDatefrom, "$lt": callDateto}}).sort({ _id: -1});
            let data = []
            for (let i = 0; i < fetchS.length; i++ ){
                let idDistrict = fetchS[i].districtId;
                let idHouse = fetchS[i].houseId;
            ...
            return res.json(data);
        }else if (diseaseId && diseaseInfoId == '' && callDatefrom && callDateto) {
            const fetchS = await model.callhistory.find({diseaseId, "createDate": {"$gte": callDatefrom, "$lt": callDateto}}).sort({ _id: -1});
            let data = []
            for (let i = 0; i < fetchS.length; i++ ){
                let idDistrict = fetchS[i].districtId;
                let idHouse = fetchS[i].houseId;
            ...
            const fetchS = await model.callhistory.find({diseaseId, diseaseInfoId, "createDate": {"$gte": callDatefrom, "$lt": callDateto}}).sort({ _id: -1});
            let data = []
            for (let i = 0; i < fetchS.length; i++ ){
                let idDistrict = fetchS[i].districtId;
                let idHouse = fetchS[i].houseId;
    ...
    async fetchAllStatistic(req, res, next){
        const fetchS = await model.callhistory.find().sort({ _id: -1});
    ...
    async getStatistic(req, res, next){
        const {id} = req.params;
        const getS = await model.callhistory.findById(id);
        return res.json(getS)
    }
}

module.exports = new statistichistoryController();