...
import { deleteSide, fetchAllSide } from '../../../http/managerAPI';
import { MANAGER_ADD_SIDE_ROUTE, MANAGER_GET_SIDE_ROUTE } from '../../../utils/consts';
import Paginations from '../../../components/Paginations';

const SideView = observer(() => {
    ...
    useEffect(()=>{
        // const interval = setInterval(() => {
            fetchAllSide().then(data => setAllItems(data));
        // }, 1000);
    ...
    return (
        <div>
            <div className="application">
                <Helmet>
                    <meta charSet="utf-8" />
                    <title>Taraflar</title>
                </Helmet>
            </div>
            <Container fluid>
                <h2 className='text-center'>Taraflar</h2>
                <Row>
                    <Form>
                        <Row>
                            <Col md={2} xs={12} className="d-flex align-items-center">
                                <Link to={MANAGER_ADD_SIDE_ROUTE}><Button variant='success'>Taraf qo`shish</Button></Link>
                            </Col>
                            <Col>
                            </Col>
                            <Col md={2} xs={12}>
                            ...
                            <tbody>
                                {
                                    currentItem.map((item,index) =>
                                        <tr key={item._id}>
                                            ...
                                            <td>{item.title}</td>
                                            <td>
                                                <Button variant="primary" onClick={() => history(MANAGER_GET_SIDE_ROUTE + '/' + item._id)}>Yangilash</Button>
