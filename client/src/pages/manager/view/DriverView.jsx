...
import { deleteDriver, fetchAllDriver } from '../../../http/managerAPI';
import { MANAGER_ADD_DRIVER_ROUTE, MANAGER_GET_DRIVER_ROUTE } from '../../../utils/consts';
import Paginations from '../../../components/Paginations';

const DriverView = observer(() => {
    ...
    useEffect(()=>{
        // const interval = setInterval(() => {
            fetchAllDriver().then(data => setAllItems(data));
        // }, 1000);
                            ...
                            <tbody>
                                {
                                    currentItem.map((item,index) =>
                                        <tr key={item._id}>
                                            <td>{cpi + index + 1}</td>
                                            <td>{item.title}</td>
                                            <td>{item.Drivertitle}</td>
                                            <td>
                                                <Button variant="primary" onClick={() => history(MANAGER_GET_DRIVER_ROUTE + '/' + item._id)}>Yangilash</Button>
