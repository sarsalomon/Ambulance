...
import { deleteEntrance, fetchAllEntrance } from '../../../http/managerAPI';
import { MANAGER_ADD_ENTRANCE_ROUTE, MANAGER_GET_ENTRANCE_ROUTE } from '../../../utils/consts';
import Paginations from '../../../components/Paginations';

const EntranceView = observer(() => {
    ...
    useEffect(()=>{
        // const interval = setInterval(() => {
            fetchAllEntrance().then(data => setAllItems(data));
        // }, 1000);
    ...
    return (
        <div>
            <div className="application">
                <Helmet>
                    <meta charSet="utf-8" />
                    <title>Kirishlar</title>
                </Helmet>
            </div>
            <Container fluid>
                <h2 className='text-center'>Kirishlar</h2>
                <Row>
                    <Form>
                        <Row>
                            <Col md={2} xs={12} className="d-flex align-items-center">
                                <Link to={MANAGER_ADD_ENTRANCE_ROUTE}><Button variant='success'>Kirish qo`shish</Button></Link>
                            </Col>
                            <Col>
                            </Col>
                            <Col md={2} xs={12}>
                            ...
                            <tbody>
                                {
                                    currentItem.map((item,index) =>
                                        <tr key={item._id}>
                                            ...
                                            <td>{item.title}</td>
                                            <td>
                                                <Button variant="primary" onClick={() => history(MANAGER_GET_ENTRANCE_ROUTE + '/' + item._id)}>Yangilash</Button>
